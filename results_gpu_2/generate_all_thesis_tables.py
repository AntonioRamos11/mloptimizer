...
HW_METRICS_DIR = os.path.join(RESULTS_DIR, 'hardware_metrics')
HW_PERF_DIR = os.path.join(RESULTS_DIR, 'hardware_performance_logs')
...
def load_experiments():
    """Load all experiment JSON files (exclude grietas_baches)."""
...
def table2_reproducibilidad(experiments):
    ...
    # Boxplot with scatter overlay (handles single-point datasets)
    fig, ax = plt.subplots(figsize=(8, 5))
    data_plot = []
    labels = []
    ds_colors = ['#3498db', '#e74c3c', '#2ecc71', '#9b59b6']
    for ds in sorted(by_ds.keys()):
        data_plot.append(by_ds[ds])
        labels.append(DATASET_DISPLAY.get(ds, ds))
    bp = ax.boxplot(data_plot, tick_labels=labels, patch_artist=True,
                    showmeans=True, meanprops=dict(marker='D', markerfacecolor='black',
                                                   markeredgecolor='black', markersize=6),
                    widths=0.5)
    for patch, c in zip(bp['boxes'], ds_colors):
        patch.set_facecolor(c)
        patch.set_alpha(0.6)
    # Overlay individual data points so single-point datasets are visible
    for i, vals in enumerate(data_plot):
        x = np.random.normal(i + 1, 0.04, size=len(vals))
        ax.scatter(x, vals, alpha=0.8, color=ds_colors[i % len(ds_colors)],
                   edgecolors='black', zorder=5, s=50,
...
def table8_benchmarks(experiments):
    ...
    # Combined bar chart — only datasets with actual results
    ds_with_data = [ds for ds in ['mnist', 'cifar10', 'fashion_mnist'] if ds in our_best]
    n_plots = len(ds_with_data)
    if n_plots > 0:
        fig, axes = plt.subplots(1, n_plots, figsize=(6 * n_plots, 6))
        if n_plots == 1:
            axes = [axes]
        for idx, ds_key in enumerate(ds_with_data):
            ax = axes[idx]
            benchmarks = BENCHMARKS.get(ds_key, [])
            ...
            if isinstance(gpu_data, list) and gpu_data:
                ...
                for record in gpu_data:
                    ...
                    for gid in metrics.get('temperature', {}):
                        temps.append(metrics['temperature'][gid])
                    for gid in metrics.get('power', {}):
                        powers.append(metrics['power'][gid].get('draw_watts', 0))
                if temps:
        ...
        for mid, mdata in models.items():
            ...
            if isinstance(gpu_data, list):
                for record in gpu_data:
                    metrics = record.get('metrics', {})
                    for gid in metrics.get('temperature', {}):
                        all_temps.append(metrics['temperature'][gid])
                    for gid in metrics.get('power', {}):
                        all_powers.append(metrics['power'][gid].get('draw_watts', 0))
