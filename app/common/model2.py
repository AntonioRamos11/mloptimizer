...
class Model:
    ...
    def build_and_train(self) -> float:
        ...
        self.model = self.build_model(input_shape, class_count)  # Assign the built model to self.model
        ...
        if self.search_space_type == SearchSpaceType.IMAGE:
            loss, training_val = self.model.evaluate(test, verbose=0)
        else:
            training_val = self.model.evaluate(test, verbose=0)
