...
sed -i "s|DATASET_NAME: str = \"\"|DATASET_NAME: str = 'mnist'|" $PARAM_FILE

# Then apply the actual values
sed -i "s|^    INSTANCE_PORT:.*|    INSTANCE_PORT: int = ${INSTANCE_PORT}|" $PARAM_FILE
sed -i "s|^    INSTANCE_MANAGMENT_URL.*|    INSTANCE_MANAGMENT_URL: str = \"${INSTANCE_MANAGMENT_URL}\"|" $PARAM_FILE
...
echo "========================================"
CLOUD_MODE=$CLOUD_MODE CUDA_VISIBLE_DEVICES=0 python -u run_master.py > logs/master.log 2>&1 &
master_pid=$!
...
for ((i=0; i<NUM_GPUS; i++)); do
  ...
  CLOUD_MODE=$CLOUD_MODE CUDA_VISIBLE_DEVICES=$i python -u run_slave.py >> logs/slave_gpu${i}.log 2>&1 &
  slave_pids[$i]=$!
