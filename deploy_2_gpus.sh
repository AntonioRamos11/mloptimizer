...
if [ "$MODE" = "cloud-slave" ]; then
    ...
    # Forzar ejecución en 2 GPUs (0, 1)
    NUM_GPUS=2
    echo_info "Imponiendo la ejecución para $NUM_GPUS GPUs de manera concurrente"
...
" 2>&1 | tee logs/dataset_predownload.log
    ...
    echo_info "Logs: logs/slave_gpu0.log .. logs/slave_gpu1.log"
    
    # Esperar a que se creen los archivos de log
    sleep 2
    tail -n 20 -f logs/slave_gpu0.log logs/slave_gpu1.log &
    TAIL_PID=$!
